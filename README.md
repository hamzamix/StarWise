...
> Version 1.2.0 changes the database volume mount from `/app/backend` to `/data` to prevent code shadowing.
> You MUST update your `docker-compose.yml` to mount the volume to `/data` and set `NODE_ENV=production`.
> Your existing data in the `starwise_db` volume will be automatically preserved and migrated.
