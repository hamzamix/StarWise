...
function App({ toggleTheme, mode }: { toggleTheme: () => void; mode: 'light' | 'dark' }) {
  ...
  const generateAllAiTags = async () => {
    setLoading(true);
    setLoadingText('Generating AI tags for all repositories... this might take some time.');
    try {
        const res = await axios.post(`${API_BASE_URL}/api/ai/generate-all-tags`);
        alert(`Successfully generated AI tags for ${res.data.count} repositories.`);
        window.location.reload();
    } catch(e) {
        console.error(e);
        alert('An error occurred while generating AI tags.');
    } finally {
        setLoading(false);
        setLoadingText('');
    }
      ...
      >
        <Toolbar>
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            <Button variant="contained" size="small" onClick={generateAllAiTags} disabled={loading} startIcon={<AutoAwesomeIcon />}>
                Generate AI Tags
             </Button>
